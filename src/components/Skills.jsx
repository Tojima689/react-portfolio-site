...
export const Skills = () => {
    ...
    useEffect(
        () => {
            ...
            axios.get('https://api.github.com/users/Tojima689/repos')
                .then((response) => {
    ...
    const convertConutToPercentage = (count) => {
        if (count > 10) {
            return 100;
        }
        return count * 10;
                        ...
                        state.requestState === requestStates.success && (
                            sortedLanguageList().map((item, index) => (
                                <div className="skill-item" key={index}>
                                    ...
                                    <CircularProgressbar value={convertConutToPercentage(item.count)} text={`${convertConutToPercentage(item.count)}%`} />
                                </div>
                            ))
                        )
                    }
